...
import static com.xqlh.heartsmart.utils.Constants.ID_COLUMN;
import static com.xqlh.heartsmart.utils.Constants.MUSIC_ID_COLUMN;
...
public class DBManager {
    ...
    public int getMusicCount(int table) {
        ...
        switch (table) {
            case Constants.LIST_ALLMUSIC:
                cursor = db.query(Constants.MUSIC_TABLE, null, null, null, null, null, null);
                break;
            case Constants.LIST_LASTPLAY:
                cursor = db.query(Constants.LAST_PLAY_TABLE, null, null, null, null, null, null);
                break;
            case Constants.LIST_MYLOVE:
                cursor = db.query(Constants.MUSIC_TABLE, null, Constants.LOVE_COLUMN + " = ? ", new String[]{"" + 1}, null, null, null);
                break;
            case Constants.LIST_MYPLAY:
                cursor = db.query(Constants.PLAY_LIST_TABLE, null, null, null, null, null, null);
                break;
        }
    ...
    public MusicInfo getSingleMusicFromMusicTable(int id) {
        ...
        db.beginTransaction();
        try {
            cursor = db.query(Constants.MUSIC_TABLE, null, ID_COLUMN + " = ?", new String[]{"" + id}, null, null, null);
            musicInfoList = cursorToMusicList(cursor);
    ...
    public List<PlayListInfo> getMyPlayList() {
        ...
        Cursor cursor = db.query(Constants.PLAY_LIST_TABLE, null, null, null, null, null, null);
        ...
        while (cursor.moveToNext()) {
            ...
            playListInfo.setName(cursor.getString(cursor.getColumnIndex(Constants.NAME_COLUMN)));
            cursorCount = db.query(Constants.PLAY_LISY_MUSIC_TABLE,null, ID_COLUMN + " = ?", new String[]{"" + id}, null,null,null);
    ...
    public void createPlaylist(String name) {
        ...
        values.put(Constants.NAME_COLUMN, name);
        db.insert(Constants.PLAY_LIST_TABLE, null, values);
    }
    ...
    public List<MusicInfo> getMusicListBySinger(String singer){
        ...
        db.beginTransaction();
        try{
            String sql = "select * from "+Constants.MUSIC_TABLE+" where "+ Constants.SINGER_COLUMN+" = ? ";
    ...
    public List<MusicInfo> getMusicListByAlbum(String album){
        ...
        db.beginTransaction();
        try{
            String sql = "select * from "+Constants.MUSIC_TABLE+" where "+ Constants.ALBUM_COLUMN+" = ? ";
    ...
    public List<MusicInfo> getMusicListByFolder(String folder){
        ...
        db.beginTransaction();
        try{
            String sql = "select * from "+Constants.MUSIC_TABLE+" where "+ Constants.PARENT_PATH_COLUMN+" = ? ";
    ...
    public ArrayList<Integer> getMusicIdListByPlaylist(int playlistId){
        ...
        try{
            String sql = "select * from "+Constants.PLAY_LISY_MUSIC_TABLE+" where "+ ID_COLUMN+" = ? ";
    ...
    public List<MusicInfo> getMusicListByPlaylist(int playlistId){
        ...
        db.beginTransaction();
        try{
            String sql = "select * from "+Constants.PLAY_LISY_MUSIC_TABLE+" where "+ ID_COLUMN+" = ? ORDER BY "+ Constants.ID_COLUMN;
            cursor = db.rawQuery(sql,new String[]{""+playlistId});
            while (cursor.moveToNext()){
    ...
    public void insertMusicInfoToMusicTable(MusicInfo musicInfo) {
        ...
        try {
            ...
            String sql = "select max(id) from " + Constants.MUSIC_TABLE + ";";
...
//			values.put("mylove",0);
            db.insert(Constants.MUSIC_TABLE, null, values);
        } catch (Exception e) {
            e.printStackTrace();
            if (cursor!=null){
                cursor.close();
    ...
    public void addToPlaylist(int playlistId,int musicId){
        ContentValues values = new ContentValues();
        values.put(Constants.ID_COLUMN,playlistId);
        values.put(Constants.MUSIC_ID_COLUMN,musicId);
        db.insert(Constants.PLAY_LISY_MUSIC_TABLE,null,values);
    }

    //检索音乐是否已经存在歌单中
    public boolean isExistPlaylist(int playlistId,int musicId){
        boolean result = false;
        Cursor cursor = db.query(Constants.PLAY_LISY_MUSIC_TABLE,null,ID_COLUMN + " = ? and "+ MUSIC_ID_COLUMN + " = ? ",
    ...
    public void deleteAllTable() {
        ...
        db.delete(Constants.MUSIC_TABLE, null, null);
        db.delete(Constants.LAST_PLAY_TABLE, null, null);
        db.delete(Constants.PLAY_LIST_TABLE, null, null);
        db.delete(Constants.PLAY_LISY_MUSIC_TABLE, null, null);
    }

    //删除指定音乐
    public void deleteMusic(int id) {
        db.execSQL("PRAGMA foreign_keys=ON");
        db.delete(Constants.MUSIC_TABLE, ID_COLUMN + " = ? ", new String[]{"" + id});
        db.delete(Constants.LAST_PLAY_TABLE, ID_COLUMN + " = ? ", new String[]{"" + id});
    }

    public void deletePlaylist(int id) {
        db.delete(Constants.PLAY_LIST_TABLE, ID_COLUMN + " = ? ", new String[]{"" + id});
    }
    ...
    public void removeMusic(int id, int witchActivity) {
        ...
        switch (witchActivity) {
            case Constants.ACTIVITY_LOCAL:
                db.delete(Constants.MUSIC_TABLE, ID_COLUMN + " = ? ", new String[]{"" + id});
                break;
            case Constants.ACTIVITY_RECENTPLAY:
                db.delete(Constants.LAST_PLAY_TABLE, ID_COLUMN + " = ? ", new String[]{"" + id});
                break;
            case Constants.ACTIVITY_MYLOVE:
                ContentValues values = new ContentValues();
                values.put(Constants.LOVE_COLUMN, 0);
                db.update(Constants.MUSIC_TABLE, null, ID_COLUMN + " = ? ", new String[]{"" + id});
                break;
        }
    }
    ...
    public int removeMusicFromPlaylist(int musicId, int playlistId) {
        ...
        int ret = 0;
        try {
            ret = db.delete(Constants.PLAY_LISY_MUSIC_TABLE, ID_COLUMN + " = ? and " + Constants.MUSIC_ID_COLUMN
    ...
    public String getMusicPath(int id) {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                path = cursor.getString(cursor.getColumnIndex(Constants.PATH_COLUMN));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public int getFirstId(int listNumber) {
        ...
        try {
            switch (listNumber) {
                case Constants.LIST_ALLMUSIC:
                    cursor = db.rawQuery("select min(id) from " + Constants.MUSIC_TABLE, null);
                    break;
                default:
    ...
    public int getNextMusic(ArrayList<Integer> musicList, int id, int playMode) {
        ...
        switch (playMode) {
            case Constants.PLAYMODE_SEQUENCE:
                ...
                break;
            case Constants.PLAYMODE_SINGLE_REPEAT:
                break;
    ...
    public ArrayList<Integer> getMusicList(int playList) {
        ...
        switch (playList) {
            case Constants.LIST_ALLMUSIC:
                ...
                break;
            case Constants.LIST_LASTPLAY:
                cursor = db.rawQuery("select * from "+Constants.LAST_PLAY_TABLE+" ORDER BY "+ Constants.ID_COLUMN,null);
                break;
            case Constants.LIST_MYLOVE:
                cursor = db.query(Constants.MUSIC_TABLE, null, Constants.LOVE_COLUMN + " = ?", new String[]{"" + 1}, null, null, null);
                break;
            case Constants.LIST_PLAYLIST:
                int listId = MyMusicUtil.getIntShared(Constants.KEY_LIST_ID);
    ...
    public void setLastPlay(int id) {
        ...
        db.beginTransaction();
        try {
            cursor = db.rawQuery("select id from " + Constants.LAST_PLAY_TABLE, null);
            while (cursor.moveToNext()) {
            ...
            if (lastList.size() < 20) {
                for (int i = 0; i < lastList.size(); i++) {
                    ...
                    db.insert(Constants.LAST_PLAY_TABLE, null, values);
                }
            } else {
                for (int i = 0; i < 20; i++) {
                    values.put(ID_COLUMN, lastList.get(i));
    ...
    public void setMyLove(int id) {
        ContentValues values = new ContentValues();
        values.put(Constants.LOVE_COLUMN, 1);
        db.update(Constants.MUSIC_TABLE, values, ID_COLUMN + " = ? ", new String[]{"" + id});
    }
...
//            values.put(DatabaseHelper.ID_COLUMN, musicInfo.getId());
            values.put(Constants.NAME_COLUMN, musicInfo.getName());
            values.put(Constants.SINGER_COLUMN, musicInfo.getSinger());
            values.put(Constants.ALBUM_COLUMN, musicInfo.getAlbum());
            values.put(Constants.DURATION_COLUMN, musicInfo.getDuration());
            values.put(Constants.PATH_COLUMN, musicInfo.getPath());
            values.put(Constants.PARENT_PATH_COLUMN, musicInfo.getParentPath());
            values.put(Constants.LOVE_COLUMN, musicInfo.getLove());
            values.put(Constants.FIRST_LETTER_COLUMN, "" + ChineseToEnglish.StringToPinyinSpecial(musicInfo.getName()).toUpperCase().charAt(0));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return values;
        ...
        try {
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    String name = cursor.getString(cursor.getColumnIndex(Constants.NAME_COLUMN));
                    String singer = cursor.getString(cursor.getColumnIndex(Constants.SINGER_COLUMN));
                    String album = cursor.getString(cursor.getColumnIndex(Constants.ALBUM_COLUMN));
                    String duration = cursor.getString(cursor.getColumnIndex(Constants.DURATION_COLUMN));
                    String path = cursor.getString(cursor.getColumnIndex(Constants.PATH_COLUMN));
                    String parentPath = cursor.getString(cursor.getColumnIndex(Constants.PARENT_PATH_COLUMN));
                    int love = cursor.getInt(cursor.getColumnIndex(Constants.LOVE_COLUMN));
                    String firstLetter = cursor.getString(cursor.getColumnIndex(Constants.FIRST_LETTER_COLUMN));
