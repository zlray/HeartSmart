...
import com.xqlh.heartsmart.ui.mine.adapter.DividerItemDecoration;
import com.xqlh.heartsmart.ui.mine.adapter.RecyclerViewAdapter;
import com.xqlh.heartsmart.utils.Constants;
import com.xqlh.heartsmart.utils.MyMusicUtil;
import com.xqlh.heartsmart.widget.MusicPopMenuWindow;
...
//        recyclerView.setItemAnimator(new DefaultItemAnimator());
        ...
        recyclerViewAdapter.setOnItemClickListener(new RecyclerViewAdapter.OnItemClickListener() {
            ...
            public void onContentClick(int position) {
                if (label != null) {
                    if (label.equals(Constants.LABEL_LAST)) {
                        MyMusicUtil.setShared(Constants.KEY_LIST, Constants.LIST_LASTPLAY);
                    } else if (label.equals(Constants.LABEL_MYLOVE)) {
                        MyMusicUtil.setShared(Constants.KEY_LIST, Constants.LIST_MYLOVE);
                    }
                }
            }
        });
            ...
            public void onClick(View v) {
                ...
                switch (playMode) {
                    case Constants.PLAYMODE_SEQUENCE:
                        playModeTv.setText(Constants.PLAYMODE_RANDOM_TEXT);
                        MyMusicUtil.setShared(Constants.KEY_MODE, Constants.PLAYMODE_RANDOM);
                        break;
                    case Constants.PLAYMODE_RANDOM:
                        playModeTv.setText(Constants.PLAYMODE_SINGLE_REPEAT_TEXT);
                        MyMusicUtil.setShared(Constants.KEY_MODE, Constants.PLAYMODE_SINGLE_REPEAT);
                        break;
                    case Constants.PLAYMODE_SINGLE_REPEAT:
                        playModeTv.setText(Constants.PLAYMODE_SEQUENCE_TEXT);
                        MyMusicUtil.setShared(Constants.KEY_MODE, Constants.PLAYMODE_SEQUENCE);
                        break;
                }
                initPlayMode();
            }
    ...
    public void updateView() {
        try {
            if (label != null) {
                if (label.equals(Constants.LABEL_LAST)) {
                    musicInfoList = dbManager.getAllMusicFromTable(Constants.LIST_LASTPLAY);
                    Log.i("lz", "最近音乐列表的大小" + musicInfoList.size());
                } else if (label.equals(Constants.LABEL_MYLOVE)) {
                    musicInfoList = dbManager.getAllMusicFromTable(Constants.LIST_MYLOVE);
                    Log.i("lz", "喜欢音乐列表的大小" + musicInfoList.size());
    ...
    public void showPopFormBottom(MusicInfo musicInfo) {
        ...
        if (label.equals(Constants.LABEL_LAST)) {
            menuPopupWindow = new MusicPopMenuWindow(LastMyloveActivity.this, musicInfo, findViewById(R.id.activity_last_mylove), Constants.ACTIVITY_RECENTPLAY);
        } else {
            menuPopupWindow = new MusicPopMenuWindow(LastMyloveActivity.this, musicInfo, findViewById(R.id.activity_last_mylove), Constants.ACTIVITY_MYLOVE);
        }
...
//      设置Popupwindow显示位置（从底部弹出）
    ...
    private void update(View swipeView, int position, MusicInfo musicInfo, boolean isDelete) {
        if (isDelete) {
            ...
            //从列表移除
            if (label.equals(Constants.LABEL_LAST)) {
                dbManager.removeMusic(musicInfo.getId(), Constants.ACTIVITY_RECENTPLAY);
            } else if (label.equals(Constants.ACTIVITY_MYLOVE)) {
                dbManager.removeMusic(musicInfo.getId(), Constants.LIST_LASTPLAY);
            }
            if (curId == musicId) {
                //移除的是当前播放的音乐
                Intent intent = new Intent(MusicPlayerService.PLAYER_MANAGER_ACTION);
